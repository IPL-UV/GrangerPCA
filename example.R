...
for (i in 4:nrow(X)) {
  X[i, 1:dxc] <- #A
    X[i, 1:dxc] + 0.2 * X[i - 2, 1:dxc] + 0.4 * Y[i - 1, ] + 0.3 * Y[i - 2, ] +
    0.2 * Y[i - 3, ] + 0.2 * Z1[i - 1, ] + 0.2 * Z1[i - 2, ]
  X[i, (dxc+1):(2*dxc)] <- #B
    X[i, (dxc+1):(2*dxc)] + 0.4 * X[i - 1, (dxc+1):(2*dxc)] + 0.5 * Z1[i - 1, ]
  X[i, (2*dxc+1):(3*dxc)] <- #C
    X[i, (2*dxc+1):(3*dxc)] + 0.1 * X[i - 2, (2*dxc+1):(3*dxc)] + 0.8 * Z1[i - 2, ] + 0.8 * mean(X[i - 1, 1:dxc]) + 0.4 * mean(X[i - 2, 1:dxc])
  X[i, (3*dxc+1):(4*dxc)] <- #D
    X[i, (3*dxc+1):(4*dxc)] + 0.1 * X[i - 2, (3*dxc+1):(4*dxc)] - 0.7 * Z2[i - 3, ]  + 0.8 * mean(X[i - 2, (2*dxc+1):(3*dxc)])
  X[i, (4*dxc+1):(5*dxc)] <- #E
...
cplot
ggsave(cplot, file = paste0(dx,"components_example1.pdf"), width = 3.5, height = 4, units = "in")
...
signalplot
ggsave(signalplot, file = paste0(dx,"signals_example1.pdf"), width = 3.5, height = 2, units = "in")
...
print(xtable(pvals, caption = "Granger causality test p-values for 
             ...
             display = c("s", rep("e",length(resall)))), booktabs = TRUE, file = paste0(dx,"pvals_ex1.tex"))
